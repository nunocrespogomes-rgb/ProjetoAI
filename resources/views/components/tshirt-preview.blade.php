@props([
    ...
    'isCart' => false,
    'isDetail' => false, 
    'size' => 'M', // Valor padrão caso não seja passado
    'cartKey' => null 
])
...
<div {{ $attributes->merge(['class' => 'relative w-full h-full flex items-center justify-center overflow-hidden rounded-lg bg-white']) }}>
    ...
    <div x-data="{ 
            ...
            currentSize: '{{ strtoupper($size ?? 'M') }}', {{-- Proteção extra aqui --}}
            get scaleClass() {
                if ('{{ $isDetail }}') return 'scale-100';
        ...
        {{-- 2. Contentor da Estampa --}}
        @if($designUrl)
            <div :class="scaleClass"
                 class="absolute inset-0 flex items-center justify-center z-20 transition-transform duration-300 {{ $isDetail ? 'p-0' : ($isCart ? 'p-1' : 'p-4') }}">
                
                <img src="{{ $designUrl }}"
                     alt="{{ $alt }}"
                     class="{{ $isDetail ? 'w-[90%] h-[90%] mt-0' : ($isCart ? 'w-[50%] h-[50%] mt-0.5' : ($scaleUp ? 'w-[55%] h-[55%] mt-1' : 'w-[40%] h-[40%] mt-2')) }} object-contain drop-shadow-xl" />
            </div>
        @endif
