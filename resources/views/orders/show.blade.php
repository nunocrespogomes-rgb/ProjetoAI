    <x-layouts::main-content
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {{-- Coluna Principal: Dados Praticados e Artigos (Ocupa 2 colunas no LG) --}}
        ...
        <div class="flex items-center gap-4 py-4 border-b border-zinc-100 dark:border-zinc-800">
            ...
            <div class="w-24 h-24 bg-zinc-100 dark:bg-zinc-950 rounded-lg flex items-center justify-center p-1 overflow-hidden">
                
                @php
                    // 1. Verificar se o item tem uma imagem personalizada associada
                    // (Ajusta o nome da relação 'tshirtImage' ou do campo se for diferente no teu projeto)
                    $designUrl = null;

                    if ($item->tshirtImage) {
                        if ($item->tshirtImage->customer_id) {
                            // É uma imagem pessoal do cliente! Usamos a rota privada que criámos
                            $designUrl = route('my_images.file', $item->tshirtImage->id);
                        } else {
                            // É uma imagem normal do catálogo público
                ...
                @if($designUrl)
                    <x-tshirt-preview 
                        :backgroundColor="$item->color->code ?? 'ffffff'" 
                        :designUrl="$designUrl" 
                        :alt="$item->tshirtImage->name ?? 'T-shirt'"
                        :size="$item->size"
                        :isCart="false"
                        class="w-full h-full"
                    />
            ...
            <div class="flex-1">
                <h4 class="text-sm font-semibold text-zinc-900 dark:text-white">
                    {{ $item->tshirtImage->name ?? 'Design Personalizado' }}
                </h4>
                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                    Tamanho: <span class="font-medium text-zinc-700 dark:text-zinc-300">{{ $item->size }}</span> | 
                    Cor: <span class="font-medium text-zinc-700 dark:text-zinc-300">{{ $item->color->name ?? 'N/A' }}</span>
                </p>
    ...
    @endforeach
                ...
                </div> {{-- <-- FECHO DA COLUNA PRINCIPAL (LG:COL-SPAN-2) --}}

                {{-- Coluna Lateral: Ações Administrativas para Funcionários e Admins (Ocupa 1 coluna no LG) --}}
            ...
            </div> {{-- <-- FECHO DA DIV GRID (GRID-COLS-1 LG:GRID-COLS-3) --}}
