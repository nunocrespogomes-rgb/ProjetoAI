    ...
    <div class="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div class="p-4 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700">
            <form method="GET" action="{{ route('customers.index') }}" class="flex items-end gap-4">
                ...
                @if($filterByName)
                    <flux:button href="{{ route('customers.index') }}" variant="ghost">Cancelar</flux:button>
                @endif
            </form>
        </div>
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center items-center gap-2">
                                    <form method="POST" action="{{ route('customers.toggle-block', $customer) }}">
                                        @csrf
                                        @method('PATCH')
                                        <flux:button type="submit" variant="ghost" size="sm" class="cursor-pointer" title="{{ $customer->blocked ? 'Desbloquear Conta' : 'Bloquear Conta' }}">
                                            @if($customer->blocked)
                                    ...
                                    <form method="POST" action="{{ route('customers.destroy', $customer) }}" onsubmit="return confirm('Tem a certeza de que deseja remover este cliente?');">
                                        @csrf
                                        @method('DELETE')
                                        <flux:button type="submit" variant="ghost" size="sm" class="cursor-pointer" title="Remover Cliente">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-zinc-400 dark:text-zinc-500">Nenhum cliente encontrado correspondente ao filtro.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
