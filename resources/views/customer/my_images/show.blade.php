<x-layouts::main-content
    ...
    <div class="p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm">
            ...
            <div class="w-32 h-32 bg-zinc-100 dark:bg-zinc-950 rounded-xl flex items-center justify-center p-1 overflow-hidden shrink-0">
    @if($designUrl)
        <x-tshirt-preview 
            :backgroundColor="$item->color->code ?? 'ffffff'" 
            :designUrl="$designUrl" 
            :alt="$item->tshirtImage->name ?? 'T-shirt'"
            :size="$item->size" {{-- <-- Certifica-te de que diz exatamente $item->size --}}
            :isCart="false"
            :isDetail="true"
            class="w-full h-full"
        />
